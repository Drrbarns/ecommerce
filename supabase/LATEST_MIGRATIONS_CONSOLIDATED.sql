-- ============================================
-- CONSOLIDATED MIGRATIONS (011-017)
-- Moolre Commerce - Production Upgrade
-- Run this AFTER the base schema and migrations 001-010
-- ============================================
...
DROP POLICY IF EXISTS "Stock adjustments viewable by service role" ON public.product_stock_adjustments;
CREATE POLICY "Stock adjustments viewable by service role" ON public.product_stock_adjustments
  FOR SELECT USING (auth.role() = 'service_role');
DROP POLICY IF EXISTS "Stock adjustments manageable by service role" ON public.product_stock_adjustments;
...
DROP POLICY IF EXISTS "Timeline viewable with order access" ON public.order_timeline;
...
DROP POLICY IF EXISTS "Timeline manageable by service role" ON public.order_timeline;
...
DROP POLICY IF EXISTS "Media viewable by everyone" ON public.media_uploads;
CREATE POLICY "Media viewable by everyone" ON public.media_uploads 
  FOR SELECT USING (true);
DROP POLICY IF EXISTS "Media manageable by service role" ON public.media_uploads;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_top_products_id ON public.top_products(id);
...
BEGIN
  ...
  REFRESH MATERIALIZED VIEW public.fulfillment_summary;
...
DROP POLICY IF EXISTS "Service role manages returns" ON public.return_requests;
...
DROP POLICY IF EXISTS "Modules viewable by everyone" ON public.store_modules;
CREATE POLICY "Modules viewable by everyone" ON public.store_modules 
  FOR SELECT USING (true);
DROP POLICY IF EXISTS "Modules editable by service role" ON public.store_modules;
...
DROP POLICY IF EXISTS "Product meta viewable by everyone" ON public.product_meta_tags;
CREATE POLICY "Product meta viewable by everyone" ON public.product_meta_tags 
  FOR SELECT USING (true);
DROP POLICY IF EXISTS "Product meta manageable by service role" ON public.product_meta_tags;
...
-- ============================================
-- COMPLETE! Next steps:
-- 1. Refresh analytics: SELECT refresh_analytics_cache();
-- 2. Verify tables: SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name IN ('product_stock_adjustments', 'order_timeline', 'media_uploads', 'return_requests', 'store_modules', 'product_meta_tags');
-- ============================================
